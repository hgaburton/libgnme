...
int test(const char *testcase, unsigned thresh)
{
    ...
    for(size_t w=x; w<nstate; w++)
    {
        ...
        wick_uscf<double,double,double> mb(orbs, orbs, enuc);
        wick_uscf<double,double,double> mb2(orbs, orbs, enuc);
        ...
        for(size_t ixb=0; ixb < v_bb.size(); ixb++)
        {
            ...
            arma::mat tmpP1a(nmo, nmo, arma::fill::zeros);
            arma::mat tmpP1b(nmo, nmo, arma::fill::zeros);
            arma::mat tmpP2aa(nmo*nmo, nmo*nmo, arma::fill::zeros);
            arma::mat tmpP2bb(nmo*nmo, nmo*nmo, arma::fill::zeros);
            arma::mat tmpP2ab(nmo*nmo, nmo*nmo, arma::fill::zeros);
            // Evaluate contribution from Wick's theory
            mb2.evaluate_rdm12(v_ba[ixa], v_bb[ixb], v_ba[iwa], v_bb[iwb], Stmp, tmpP1a, tmpP1b, tmpP2aa, tmpP2bb, tmpP2ab);
            // Increment total 2RDM
            RDM1(x,w) += (tmpP1a + tmpP1b) * V(w)(iwa, iwb) * V(x)(ixa, ixb);
            // Increment total 1RDM
            RDM2(x,w) += (tmpP2aa + tmpP2bb + tmpP2ab) * V(w)(iwa, iwb) * V(x)(ixa, ixb);
        }
